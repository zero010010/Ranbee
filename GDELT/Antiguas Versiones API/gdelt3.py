...
def endpoint_limpiar_dataframe(funcion_obtener_datos: str, keyword: str, country: str): # LOS PARAMETROS NECESARIOS
    ...
    carpeta = f"carpeta_{funcion_obtener_datos}_limpios"  # Creamos una carpeta para unificar los datos limpios para ada endpoint
    if not os.path.exists(carpeta):
        os.makedirs(carpeta)
    
    file_number = 1
    while os.path.exists(f"{carpeta}/csv_{funcion_obtener_datos}_limpios_{file_number}.csv"):
        file_number += 1
    filename = f"{carpeta}/csv_{funcion_obtener_datos}_limpios_{file_number}.csv" 
...
# ULTIMO CAMBIO, ARREGLO DE LA CREACION DE CARPETAS EN EL ENDPOINT LIMPIAR DATAFRAME. He tenido que modificar 
# el código de los otros endpoints para incluir dentro del condicional de descarga la creacion de las carpetas