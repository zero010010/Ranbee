...
@app.get("/eda/")
def perform_eda(data_source: str, keyword: str, country: str, start_date: str, end_date: str):
    # Verifica el valor de data_source y obtiene el DataFrame correspondiente
    if data_source == "articles":
        df = get_articles(keyword, country, start_date, end_date, "no")
    elif data_source == "tone":
        df = get_tone_results(keyword, country, start_date, end_date, "no")
    elif data_source == "popularity":
        df = get_popularity_results(keyword, country, start_date, end_date, "no")
    else:
        # Si data_source no es válido, devuelve un error
        return {"error": "Invalid data source"}

    # Realiza el análisis exploratorio de datos (EDA) en el DataFrame
    eda_results = {
